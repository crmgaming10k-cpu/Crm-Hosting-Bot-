...
app = Flask('')
NAME='@Nxt_Hosting_ReBot'  #Add what ever you want to desplay in bot like , i am ____ hostin bot 
@app.route('/')
def home():
...
# --- Configuration ---
TOKEN = '8961186919:AAHcL6e61GStUI02lAXpcGgMzIFTTN0RCHs'
...
UPDATE_CHANNEL = '@NXT_B2NN4R_OFFICIAL' # ADD YOUR YOU CHANNEL USERNAME 

# ================= PASSWORD SYSTEM =================
MASTER_PASSWORD = os.getenv("MASTER_PASSWORD", "Python")  # 👈 CHANGE "Python" TO YOUR PASSWORD
...
def check_password_attempts(user_id):
    """Check if user is locked out"""
    if user_id in AUTH_LOCKOUT:
        lockout_time = AUTH_LOCKOUT[user_id]
        if time.time() - lockout_time < LOCKOUT_TIME:
            remaining = int(LOCKOUT_TIME - (time.time() - lockout_time))
            return False, remaining
        else:
            # Lockout expired
            del AUTH_LOCKOUT[user_id]
...
def authenticate_user(user_id, entered_password):
    ...
    # Check lockout
    can_try, remaining = check_password_attempts(user_id)
    if not can_try:
        mins = remaining // 60
        secs = remaining % 60
        return False, f"🔒 Locked out! Try again in {mins}m {secs}s"
    ...
    if entered_password == MASTER_PASSWORD:
        ...
        if remaining <= 0:
            AUTH_LOCKOUT[user_id] = time.time()
            return False, f"🚫 Too many failed attempts! Locked for 30 minutes."
...
def command_start_with_password(message):
    user_id = message.from_user.id
    ...
    if not can_try:
        mins = remaining // 60
        secs = remaining % 60
        bot.reply_to(message, f"🔒 **Locked Out!**\n\nTry again in {mins}m {secs}s", parse_mode="HTML")
        return
    
    # Ask for password
    msg = bot.reply_to(message, 
        f"🔐 <b>Password Required!</b>\n\n"
        f"Enter password to use this bot.\n\n"
        f"<i>⚠️ {MAX_ATTEMPTS} attempts allowed</i>",
        parse_mode="HTML"
    )
...
def process_password(message):
    ...
    if entered.lower() == '/cancel':
        bot.reply_to(message, "❌ Cancelled. Send /start again to try.")
        return
    
    success, msg = authenticate_user(user_id, entered)
    ...
    if success:
        ...
        bot.reply_to(message, msg, parse_mode="HTML")
        # Allow retry
        msg2 = bot.reply_to(message, "🔐 Enter password again or /cancel:")
        bot.register_next_step_handler(msg2, process_password)
...
def require_auth(func):
    """Decorator to check authentication before executing command"""
    def wrapper(message):
        user_id = message.from_user.id
        
        if PASSWORD_ENABLED and not is_authenticated(user_id):
            # Check lockout
            can_try, remaining = check_password_attempts(user_id)
            if not can_try:
                mins = remaining // 60
                secs = remaining % 60
                bot.reply_to(message, f"🔒 Locked out! Try again in {mins}m {secs}s", parse_mode="HTML")
                return
            
            msg = bot.reply_to(message, "🔐 Enter password to continue:")
            bot.register_next_step_handler(msg, process_password_and_continue, func, message)
            return
        
        return func(message)
    return wrapper

def process_password_and_continue(message, func, original_message):
    """Process password and continue with original command"""
    user_id = message.from_user.id
    entered = message.text.strip()
    ...
    if success:
        bot.reply_to(message, msg, parse_mode="HTML")
        func(original_message)
    else:
        bot.reply_to(message, msg, parse_mode="HTML")
        msg2 = bot.reply_to(message, "🔐 Enter password again or /cancel:")
        bot.register_next_step_handler(msg2, process_password_and_continue, func, original_message)
